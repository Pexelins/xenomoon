...
// hive to recalibrate ("give the builder the research skill"). It edits the framework registry — the
...
export function makeSetSkillTool() {
  return tool(
    ...
    "Assign or unassign a FRAMEWORK skill to/from an agent's skill index — use ONLY when the user " +
      "explicitly asks to change which skills an agent has (e.g. 'give the builder the research " +
      ...
      "only; the hive's own skills are fixed.",
    {
      agent: z
        .string()
        .describe("The framework agent to recalibrate, e.g. art-director, asset-advisor."),
      skill: z.string().describe("The framework skill name, e.g. research, screen-effects."),
