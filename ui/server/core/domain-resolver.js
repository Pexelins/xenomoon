 ...
 * @property {boolean} populated whether the domain ships pre-baked capabilities or
 ...
 *           external binary the verify gate resolves/exports vs. package scripts (Node)
 ...
 *           rules) — true only for a domain that needs real files at a project path.
...
function normalizeDescriptor(raw, name) {
  ...
  return {
    ...
    // capabilities declares populated:true and so stays under doctor's hard checks.
    populated: raw.populated === true,
    engine: {
      name: /** @type {string} */ (engineName),
      projectFile: /** @type {string} */ (projectFile),
      // Optional: does this engine run via an external binary vs package scripts
      // (Node)? Default false — a new domain needs no engine-binary probe.
    ...
    // a domain that needs real files at a project path opts in with true.
