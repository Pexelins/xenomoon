	...
	weapon_manager.ammo_manager.ammo_dict[current_weapon.resources.ammo_type] >= current_weapon.resources.nb_proj_shots_at_same_time)
		...
		for i in range(current_weapon.resources.nb_proj_shots):
			...
			weapon_manager.ammo_manager.ammo_dict[current_weapon.resources.ammo_type] >= current_weapon.resources.nb_proj_shots_at_same_time):
				...
				# No shoot animation assigned — non-fatal, fire proceeds without anim.
...
func get_camera_fov() -> Vector3:
	var camera : Camera3D = get_node("../../CameraHolder/CameraRecoilHolder/Camera")
	# Use the camera's own viewport rect — correct regardless of window stretch mode
	# or SubViewport nesting. Never read get_window().content_scale_size here.
	...
	var raycast_start : Vector3 = camera.project_ray_origin(viewport_size / 2.0)
	var raycast_end : Vector3 = Vector3.ZERO
	if current_weapon.resources.type == current_weapon.resources.types.HITSCAN: raycast_end = raycast_start + camera.project_ray_normal(viewport_size / 2.0) * current_weapon.resources.max_range
	if current_weapon.resources.type == current_weapon.resources.types.PROJECTILE: raycast_end = raycast_start + camera.project_ray_normal(viewport_size / 2.0) * 280
